resource "aws_iam_policy" "AmazonEKSClusterCloudWatchMetricsPolicy" {
  name   = "${var.region}-eks-${var.name}-AmazonEKSClusterCloudWatchMetricsPolicy"
  policy = <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_policy" "AmazonEKSClusterNLBPolicy" {
  name   = "${var.region}-eks-${var.name}-AmazonEKSClusterNLBPolicy"
  policy = <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role" "eks_cluster_role" {
  name                  = "${var.region}-eks-${var.name}-cluster-role"
